...
const shapes={horizontal:{w:1330,h:460,body:word('CUT',0,80)+p(slash,350,0)+word('SHORT',596,80)},stacked:{w:734,h:850,body:word('CUT',0,80)+p(slash,420,0)+word('SHORT',0,500)},slash:{w:247,h:460,body:p(slash)}};
...
function svg(shape,theme,pad=80){const s=shapes[shape],c=colors[theme],w=s.w+pad*2,h=s.h+pad*2;return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img" aria-label="CUT/SHORT${shape==='slash'?' slash symbol':''}"><title>CUT/SHORT</title>${c.bg?`<path fill="${c.bg}" d="M0 0H${w}V${h}H0Z"/>`:''}<g fill="${c.fg}" transform="translate(${pad} ${pad})">${s.body}</g></svg>`;}
...
for(const theme of ['primary','blue','pink']){
 ...
 const scale=(w-160)/1330,y=(h-460*scale)/2;
...
let board='<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1200" viewBox="0 0 1600 1200">';
for(const [i,theme] of ['primary','blue','pink'].entries()){const c=colors[theme];board+=`<path fill="${c.bg}" d="M0 ${i*400}H1600V${i*400+400}H0Z"/><g fill="${c.fg}" transform="translate(215 ${i*400+40}) scale(.88 .69565)">${shapes.horizontal.body}</g>`;}
// Preserve proportions in the actual preview.
board='<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1800">'+['primary','blue','pink'].map((theme,i)=>`<path fill="${colors[theme].bg}" d="M0 ${i*600}H1600V${(i+1)*600}H0Z"/><g fill="${colors[theme].fg}" transform="translate(135 ${i*600+70})">${shapes.horizontal.body}</g>`).join('')+'</svg>';
...
fs.writeFileSync(path.join(out,'manifest.json'),JSON.stringify({brand:'CUT/SHORT',primary:'Black on electric yellow',secondary:['White on cobalt blue','Black on hot pink'],pngExports:manifest},null,2)+'\n');
